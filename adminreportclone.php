...
$combinedDatein = $yearin . '-' . str_pad($monthin, 2, '0', STR_PAD_LEFT); // Combine year and month with a hyphen separator

$combinedDateout = $yearout . '-' . str_pad($monthout, 2, '0', STR_PAD_LEFT); // Combine year and month with a hyphen separator

$dmin = date('F-d-Y', strtotime($combinedDatein));
$dmax = date('F-d-Y', strtotime($combinedDateout));
...
WHERE check_in BETWEEN '$combinedDatein' AND  '$combinedDateout'
...
background: radial-gradient(circle, rgba(0,228,255,1) 0%, rgba(0,130,188,1) 50%);
  ...
  <a href="admin.php" style="float: left;">
    ...
    <h4>RESERVATION SUMMARY REPORT</h4>
    DATE FROM: <?php echo $dmin; ?> DATE TO: <?php echo $dmax; ?> 
